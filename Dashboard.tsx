...
const Dashboard: React.FC<DashboardProps> = ({ status, insights, onToggle }) => {
  return (
    <div className="p-6 h-full flex flex-col gap-6 overflow-y-auto pb-20">
      ...
      <section>
        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 px-1">Service Core (Android)</h3>
      ...
      <section className="bg-slate-900/50 rounded-2xl p-5 border border-slate-800">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[9px] text-slate-600 font-mono">SYSTEM LOGS</span>
        </div>
          ...
          {insights.map(insight => (
            <div key={insight.id} className="flex gap-4 group">
              ...
              <div>
                ...
                <span className="text-[9px] text-slate-600 font-mono mt-1 block uppercase">
...
const StatusCard: React.FC<{title: string, active: boolean, onToggle: () => void, description: string}> = ({title, active, onToggle, description}) => (
  <div className={`bg-slate-900 border rounded-2xl p-4 flex items-center justify-between transition-all group ${active ? 'border-slate-800 hover:border-blue-500/30' : 'border-slate-800/50 grayscale opacity-60'}`}>
    <div className="flex-1">
      <h3 className="font-bold text-sm tracking-tight group-hover:text-blue-400 transition-colors">{title}</h3>
    ...
    >
      <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-300 ${active ? 'left-7 scale-110' : 'left-1'}`} />
    </button>
  </div>
);
